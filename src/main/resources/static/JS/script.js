...
function renderCartRow(item) {
    ...
    <td>
      <a class="detail-link" href="${buildDetailUrl(item)}" data-pid="${item.productId}">
        <img src="${item.imageUrl || "img/FruitsImg/null/G2_0.jpg"}" alt="${item.name || ""}"/>
      </a>
    </td>
    <td>
      <a class="detail-link" href="${buildDetailUrl(item)}" data-pid="${item.productId}">
...
(function bindQtyControlsForSingle() {
    ...
    (function initSingleProductAddToCart() {
        ...
        btn.addEventListener("click", async () => {
            ...
            // ★ 讀取當前頁面顯示的單價
            const currentUnitPrice = document.querySelector(".price")?.textContent || "";
...
window.formatCurrency = window.formatCurrency || (n =>
        new Intl.NumberFormat('zh-TW', { style: 'currency', currency: 'TWD' }).format(Number(n||0))
);
...
(function initSingleProductAddToCart_ImageCapture(){
    ...
    const mainImg = document.getElementById('MainImg') || document.querySelector('.main-img,.product-main img');
...
(function applyListImagesByRule(){
    ...
    cards.forEach(card=>{
        ...
        const img = card.querySelector('img.product-img') || card.querySelector('img');
        if (!img) return;
        ...
        img.addEventListener('error', () => {
            ...
            if (img.src !== location.origin + '/' + fallback && !img.src.endsWith(fallback)) {
                img.src = fallback;
            } else {
...
(function enhanceRenderCartRow(){
    ...
    function ensureImgOnRow(tr, item){
        ...
        if (img) {
            ...
            td.innerHTML = `<img src="${url}" alt="${item.name||''}" width="80" height="80" style="object-fit:cover">`;
    ...
    function renderCartRow(item) {
        ...
        <td><img src="${item.imageUrl || "img/default.png"}" alt="${item.name}" /></td>
        ...
        <td>
            <input type="number" class="cart-qty" data-id="${item.productId}" value="${item.quantity ?? 1}" min="1">
        </td>
    ...
    window.renderCartRow = function(item){
        ...
        // 把圖片設為我們的優先序來源
        try { ensureImgOnRow(tr, item); } catch(e){ console.warn('ensureImgOnRow failed:', e); }
        return tr;
    };
})();
...
(async function fallbackCartRenderIfNeeded(){
    const tbody = document.getElementById('cart-tbody') || document.getElementById('cartTableBody');
    if (!tbody) return;
...
function pickPriceForQty(data, qty, t1 = DEFAULT_T1, t2 = DEFAULT_T2, {
    smallPolicy = 'highest', midPolicy = 'highest', largePolicy = 'lowest',
} = {}) {
    const tier = chooseTier(qty, t1, t2);
    const m1 = data?.m1?.[tier];
    const m2 = data?.m2?.[tier];
    const picked = (m1 == null && m2 == null) ? { price:null, source:null }
        : (m1 == null) ? { price:m2, source:'M2' }
            : (m2 == null) ? { price:m1, source:'M1' }
                : (tier === 'low') ? ((m1 <= m2) ? {price:m1,source:'M1'} : {price:m2,source:'M2'})
                    : ((m1 >= m2) ? {price:m1,source:'M1'} : {price:m2,source:'M2'});
...
function renderPrice(root, unitPrice, qty) {
    ...
    if (totalEl) totalEl.textContent = (total == null) ? '—' : formatCurrency(total);
}
...
async function hydrateListingCardPrices() {
    ...
    for (const card of cards) {
        ...
        try {
            ...
            const { unitPrice, sourceMarket } = pickPriceForQty(tiers, 1, DEFAULT_T1, DEFAULT_T2, {
                smallPolicy: 'highest', midPolicy: 'highest', largePolicy: 'lowest'
            });
...
async function initSingleProductPrice() {
    ...
    function apply(tiers) {
        ...
        const { unitPrice, sourceMarket, tier } = pickPriceForQty(tiers, qty, t1, t2, {
            smallPolicy: 'highest', midPolicy: 'highest', largePolicy: 'lowest'
        });
    ...
    try {
        ...
        if (!document.querySelector('.price')) {
            ...
            h.innerHTML = '單價：<span class="price" aria-live="polite">載入中…</span> <small class="unit"></small>';
            (document.querySelector('.single-pro-details') || document.body).prepend(h);
            const subtotalP = document.createElement('p');
            subtotalP.className = 'subtotal';
            subtotalP.innerHTML = '小計：<span class="total-price">—</span>';
            (document.querySelector('.single-pro-details') || document.body).appendChild(subtotalP);
        }
        apply(tiers);
...
document.addEventListener('DOMContentLoaded', () => {
    try { hydrateListingCardPrices(); } catch(e) { console.warn(e); }
    try { initSingleProductPrice(); } catch(e) { console.warn(e); }
});
/* ======== End Pricing Module ======== */
...
function recalcTotals() {
    const tbody = getCartTbody?.() || document.querySelector('#cart tbody, #cart-tbody');
    if (!tbody) return;
